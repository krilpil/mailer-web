...
- `GET /api/batch_mail/task/list` возвращает только задачи, принадлежащие текущему аккаунту:
  - по `template_id` из `account_template`;
  - по `group_id` из `account_recipient`;
  - по `addresser` (почтовый адрес/домен) из `account_mailbox` и `account_domain`.
